...
do_install() {
	...
	echo "OASIS ${DISTRO_VERSION} (Core edition)" > ${D}${sysconfdir}/oasis-version
	echo "Built from branch: ${METADATA_BRANCH}" >> ${D}${sysconfdir}/oasis-version
	echo "Revision: ${METADATA_REVISION}" >> ${D}${sysconfdir}/oasis-version
	echo "Target system: ${TARGET_SYS}" >> ${D}${sysconfdir}/oasis-version

	echo "${@get_layers(bb, d)}" > ${D}${sysconfdir}/oasis-build-info

	echo "VERSION=\"${DISTRO_VERSION}\"" > ${D}${sysconfdir}/os-release
	echo "VERSION_ID=\"${DISTRO_VERSION}\"" > ${D}${sysconfdir}/os-release
	echo "NAME=\"OASIS\"" >> ${D}${sysconfdir}/os-release
	echo "ID=\"oasis\"" >> ${D}${sysconfdir}/os-release
	...
	echo "HOME_URL=\"http://www.oasis-distribution.org\"" >> ${D}${sysconfdir}/os-release
